...
A file called `app.R` should appear in RStudiom and RStudio will have created a new directory with the app name that you specified (e.g. `test`). The `app.R` script defines the UI (user interface), app layout, code run by the server and ends with an instruction to run the app. Note that you might see some older Shiny apps that were defined in two separate files (`UI.R` and `server.R`). If you view this file in RStudio the option to run the app will appear in the top toolbar. Hitting the **Run App** Button will create a new RStudio window this example app. Congratulations, you have just created your first shiny app `r emo::ji("tada")`
...
- The UI part of the script defines an argument `bins` that is used to define the parameters of how the histogram is displayed. This is the basic principle of Shiny. 
...
TML elements can be added by a number of functions that parallel common HTML tags. e.g. `h1`, `h2` for headers and `a` to create a hyperlink. The `helpText` function can also be used to add explanatory text below a particular input option. In the following code we add headers, some help text and a hyperlink to a reference about histograms. Images can also be added using the `img` function.
...
Sometimes you might want to use some kind of control structure to translate the input parameters into the arguments for plotting. 

```{r eval=FALSE}
  
  colour <- input$colour
  # For some bizarre reason we might want to confuse the user!
  if (colour == "Red"){
    mycolour <- "green"
...
We can add new outputs by adding new arguments to `mainPanel`. Let's say we want to show a boxplot of the data in addition to the histogram. First we add a call to `plotOutput` in the `mainPanel` code.
...
We then need to modify the server script to create the `boxPlot` object that we want to display. Having already created a histogram, we should just need a function that runs the command `boxplot(x)` and assigns the value to `output$boxPlot`.

However, the following code is incorrect. Can you work out why? `r emo::ji("thinking")`
